...
@Component({
  selector: 'app-licencia-conducir',
  templateUrl: './certificado-antecedentes-transito.component.html',
  styleUrls: ['./certificado-antecedentes-transito.component.css'],
  providers: [MessageService]
})
export class CertificadoAntecedentesComponent implements OnInit {
  cols: any[];
  idPersonal: number;
  nombre: string;
  ...
  ngOnInit(): void {
    ...
    this._datosPersonalesService.getPersonalById(this.idPersonal).subscribe(res => {
      ...
      this.findUrl = {
        url: `../Uploads/${this.nombre}/CERTIFICADO DE ANTECEDENTES TRANSITO-TRANSPORTE Y SEG. VIAL (SOLO CHOFERES)` 
      }
  ...
  preview(files: FileList) {
    ...
    let url = {
      "tipoProceso": "CERTIFICADO DE ANTECEDENTES TRANSITO-TRANSPORTE Y SEG. VIAL (SOLO CHOFERES)",
