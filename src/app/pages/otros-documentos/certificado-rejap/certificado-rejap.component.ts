...
@Component({
  selector: 'app-certificado-rejap',
  templateUrl: './certificado-rejap.component.html',
  styleUrls: ['./certificado-rejap.component.css'],
  providers: [MessageService]
})
export class CertificadoRegapComponent implements OnInit {
  cols: any[];
  idPersonal: number;
  nombre: string;
  ...
  ngOnInit(): void {
    ...
    this._datosPersonalesService.getPersonalById(this.idPersonal).subscribe(res => {
      ...
      this.findUrl = {
        url: `../Uploads/${this.nombre}/CERTIFICADO DE ANTECEDENTES PENALES REJAP`
      }
      this._datosPersonalesService.getPersonalImg(this.findUrl).subscribe(res => {
        if (res.array == undefined) {
          ...
          // Si esta vacio la carpeta hace un delete de la tabla public.sdp_documentos de REJAP
  ...
  preview(files: FileList) {
    ...
    let url = {
      "tipoProceso": "CERTIFICADO DE ANTECEDENTES PENALES REJAP",
