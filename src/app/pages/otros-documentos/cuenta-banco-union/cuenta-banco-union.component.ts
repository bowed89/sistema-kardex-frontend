...
@Component({
  selector: 'app-cuenta-banco-union',
  templateUrl: './cuenta-banco-union.component.html',
  styleUrls: ['./cuenta-banco-union.component.css'],
  providers: [MessageService]
})
export class CuentaBancoUnionComponent implements OnInit {
  cols: any[];
  idPersonal: number;
  nombre: string;
  ...
  ngOnInit(): void {
    ...
    this._datosPersonalesService.getPersonalById(this.idPersonal).subscribe(res => {
      ...
      this.findUrl = {
        url: `../Uploads/${this.nombre}/REGISTRO CUENTA BANCO UNION`
      }
  ...
  preview(files: FileList) {
    ...
    let url = {
      "tipoProceso": "REGISTRO CUENTA BANCO UNION",
