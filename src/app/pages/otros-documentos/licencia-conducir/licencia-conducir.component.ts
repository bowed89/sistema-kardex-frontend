...
@Component({
  ...
  templateUrl: './licencia-conducir.component.html',
  styleUrls: ['./licencia-conducir.component.css'],
...
export class LicenciaConducirComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._datosPersonalesService.getPersonalById(this.idPersonal).subscribe(res => {
      ...
      this.findUrl = {
        url: `../Uploads/${this.nombre}/LICENCIA DE CONDUCIR Y FOTOCOPIA DE CI GARANTES (SOLO CHOFERES)`
      }
  ...
  preview(files: FileList) {
    ...
    let url = {
      "tipoProceso": "LICENCIA DE CONDUCIR Y FOTOCOPIA DE CI GARANTES (SOLO CHOFERES)",
