...
@Component({
  selector: 'app-registro-nua-cua-afp',
  templateUrl: './registro-nua-cua-afp.component.html',
  styleUrls: ['./registro-nua-cua-afp.component.css'],
  providers: [MessageService]
})
export class RegistroNuaCuaAfpComponent implements OnInit {
  cols: any[];
  idPersonal: number;
  nombre: string;
  ...
  ngOnInit(): void {
    ...
    this._datosPersonalesService.getPersonalById(this.idPersonal).subscribe(res => {
      ...
      this.findUrl = {
        url: `../Uploads/${this.nombre}/REGISTRO NUA-CUA AFP`
      }
  ...
  preview(files: FileList) {
    ...
    let url = {
      "tipoProceso": "REGISTRO NUA-CUA AFP",
