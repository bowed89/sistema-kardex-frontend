...
@Component({
  selector: 'app-certificado-sipase',
  templateUrl: './certificado-sipase.component.html',
  styleUrls: ['./certificado-sipase.component.css'],
  providers: [MessageService]
})
export class CertificadoSipaseComponent implements OnInit {
  cols: any[];
  idPersonal: number;
  nombre: string;
  ...
  ngOnInit(): void {
    ...
    this._datosPersonalesService.getPersonalById(this.idPersonal).subscribe(res => {
      ...
      this.findUrl = {
        url: `../Uploads/${this.nombre}/CERTIFICADO DE NO VIOLENCIA SIPASE`
      }
      this._datosPersonalesService.getPersonalImg(this.findUrl).subscribe(res => {
        if (res.array == undefined) {
          ...
          console.log('body de sipase', body);
          this._datosPersonalesService.deleteDocumento(body).subscribe(res => {
            console.log(res);
          })
        } else {
          ...
          console.log('res nombres de archivos ==> ', res.fileNames2);
          this.listadoArchivos = res.fileNames2;
          this.listadoArchivos2 = res.response;

          // Si esta vacio la carpeta hace un delete de la tabla public.sdp_documentos de SIPASE
  ...
  preview(files: FileList) {
    ...
    let url = {
      "tipoProceso": "CERTIFICADO DE NO VIOLENCIA SIPASE",
    ...
    this._datosPersonalesService.postSubirImg(this.formData).subscribe(res => {
      console.log(res);
      // registramos en la tabla public.sdp_documentos
      this._datosPersonalesService.postDocumentos(body).subscribe(res => {
        // luego de subir el archivo se desabilita los btns para no volver a subir otros archivos
  ...
  registroFecha(evento) {
    if (evento) {
      this.disableSubir = true;
      this.fechaRegistro = evento;
    }
  }
