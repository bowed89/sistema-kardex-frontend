...
@Component({
  selector: 'app-evaluacion-desempenio',
  templateUrl: './evaluacion-desempenio.component.html',
  styleUrls: ['./evaluacion-desempenio.component.css'],
  providers: [MessageService]
})
export class EvaluacionDesempenioComponent implements OnInit {
  cols: any[];
  idPersonal: number;
  nombre: string;
  ...
  ngOnInit(): void {
    ...
    this._datosPersonalesService.getPersonalById(this.idPersonal).subscribe(res => {
      ...
      this.findUrl = {
        url: `../Uploads/${this.nombre}/EVALUACION DEL DESEMPEÑO`
      }
  ...
  preview(files: FileList) {
    ...
    let url = {
      "tipoProceso": "EVALUACION DEL DESEMPEÑO",
