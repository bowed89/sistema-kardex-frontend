...
@Component({
  selector: 'app-estudios-realizados-listado',
  templateUrl: './estudios-realizados-listado.component.html',
  styleUrls: ['./estudios-realizados-listado.component.css']
})
export class EstudiosRealizadosListadoComponent implements OnInit {
  cols: any[];
  personal: any;
  selectedCar: any;
  idGrupo: string;
  ...
  ngOnInit(): void {
    ...
    if (this.idGrupo == '1') {
      ...
      this.extension.push(
        ...
        { label: 'FORMACIÓN ACADÉMICA', value: 'formacion-academica' },
        { label: 'CERTIFICADOS DE CURSOS', value: 'certificados-cursos' })
  ...
  seleccionarAccion(value, id) {
    ...
    console.log('id', id);
    if (value == 'formacion-academica') {
      this.router.navigate([`/formacion-academica/${id}`])
    }
    if (value == 'certificados-cursos') {
      this.router.navigate([`/certificados-cursos/${id}`])
    }
  }
